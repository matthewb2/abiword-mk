...
#include "pwl.h"
//pascal #include "unused-parameter.h"
...
//pascal #include "relocatable.h"
//pascal #include "configmake.h"
...
{
	//pascal char *newpath = (char *) relocate (path);
	char *newpath = enchant_get_user_config_dir ();
	if (path == newpath)
...
enchant_get_user_config_dir (void)
{
		...
		/* Dynamically locate executable and return containing directory */
        ...
        char * config_dir = NULL;
            ...
            if(GetModuleFileNameW(NULL,dll_path,MAX_PATH))
			{
				...
				g_free(utf8_dll_path);
					...
					if(prefix)
                    {
                    config_dir = g_build_filename(prefix,"dict",NULL);
                    g_free(prefix);
                    return config_dir;
                    }
...
enchant_load_providers_in_dir (EnchantBroker * broker, const char *dir_name)
{
            ...
            //EnchantProviderInitFunc init_func;
                            ...
                            provider = init_enchant_provider ();
...
{
//pascal char *module_dir = enchant_relocate (PKGLIBDIR "-" ENCHANT_MAJOR_VERSION);
//	char *module_dir = enchant_relocate ("PKGLIBDIR-ENCHANT_MAJOR_VERSION");

    char *module_dir = enchant_get_user_config_dir ();
