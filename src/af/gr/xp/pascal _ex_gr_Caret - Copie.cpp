...
GR_Caret::GR_Caret(GR_Graphics * pG)
	:  	m_xPoint(0), // init the x and y point to some value, since we don't have a sane value here
		...
		m_blinkTimeout(NULL),
		m_nDisableCount(1),
		m_bCursorBlink(true),//pascal
		m_bCursorIsOn(false),//pascal
...
void GR_Caret::enable()
{
	...
	{
		UT_DEBUGMSG(("Don't emable : disable Count is already zero \n"));
		return;
	}
	...
	if (m_nDisableCount != 0)
	{
		UT_DEBUGMSG(("Don't emable : disable Count has not reached zero \n"));
		return;
	}
...
void GR_Caret::JustErase(UT_sint32 xPoint,UT_sint32 yPoint)
{
       if(m_bRecursiveDraw)
       {
	    UT_DEBUGMSG(("Doing recursive Just Erase! - abort \n"));
	    return;
       }
       ...
       {
	    UT_DEBUGMSG(("Doing Just Erase! now \n"));
...
void GR_Caret::_blink(bool bExplicit)
{
	if(m_bRecursiveDraw)
	{
	    UT_DEBUGMSG(("Doing recursive blink! - abort \n"));
	    return;
	}
	if (!m_bPositionSet)
		return;
	...
	{
		m_iRetry++;
		UT_DEBUGMSG(("Caret redraw after %d ms \n",time_between));
	}
	else
	{
		m_iRetry = 0;
	...
	*/
	...
	if (bExplicit || _getCanCursorBlink () || !m_bCursorIsOn)
	{
		...
		if (m_bCursorIsOn)
		{
			UT_DEBUGMSG(("Clear Caret reTry %d \n",m_iRetry));
			m_pG->restoreRectangle(m_iCaretNumber*3+0);
		...
		else
		{
			...
			if(m_bCaret1OnScreen)
			{
				...
				UT_DEBUGMSG(("Draw Caret reTry %d \n",m_iRetry));
...
{
	UT_DEBUGMSG(("Pending blink set in GR_CARET \n"));
