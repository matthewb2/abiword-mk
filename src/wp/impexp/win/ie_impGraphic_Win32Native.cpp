...
UT_Error IE_ImpGraphic_Win32Native::importGraphic(/*UT_ByteBuf* pBB*/ const UT_ConstByteBufPtr & pBB,
												  FG_ConstGraphicPtr & pfg)
{
	std::string mimetype;
	UT_Error err;
    //pascal UT_Error err = _convertGraphic(pBB, mimetype);
	...
	{
		//pascal if(!pFGR->setRaster_JPEG(m_pBB))
		{
	...
	{
		//pascal if(!pFGR->setRaster_PNG(m_pBB))
		{
			return UT_IE_FAKETYPE;
		}
	}
